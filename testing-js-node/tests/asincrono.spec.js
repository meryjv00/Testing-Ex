...
describe('Asincrono - Callback', () => {
  test('Callback', (done) => {
    ...
    asincronia.callback(callbackInterno);
  });
});
...
describe('Asincrono - Promise(resolve, reject)', () => {
  ...
  test('Promise - Promise(resolve, reject)', (done) => {
    ...
    asincronia.axiosGet(url).then((datos) => {
      const data = {
        id: 1,
        title: 'Post 1',
      };
  ...
  test('Promise - .resolves', () => {
    ...
    return expect(asincronia.axiosGet(url)).resolves.toEqual({ name: 'typicode' });
  });

  // Probando que falla
  test('Promise - .rejects', () => {
    const url = 'https://my-json-server.typicode.com/typicode/demo/fails';
    return expect(asincronia.axiosGet(url)).rejects.toEqual('Request failed with status code 404');
  });
...
describe('Asíncrono usando async / await', () => {
  ...
  test('Probando async / await - OK', async () => {
    ...
    const post = await asincronia.axiosGet(postAPI);
    const comments = await asincronia.axiosGet(commnetsAPI);
  ...
  test('Probando async / await - FAIL', async () => {
    ...
    await expect(asincronia.axiosGet(failAPI)).rejects.toEqual('Request failed with status code 404');
  });
